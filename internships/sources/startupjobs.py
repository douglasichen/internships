...
Listings come from the site's public Algolia index, hit directly via the
same multi-query endpoint the frontend calls -- no HTML to scrape. The
x-algolia-api-key in API_URL is a public, read-only, index-restricted
frontend key (see restrictIndices in its own payload), same as any key
visible in the site's browser network tab.
...
NAME = "startupjobs"
API_URL = ("https://4cqmtmmk73-3.algolianet.com/1/indexes/*/queries"
           "?x-algolia-agent=Algolia%20for%20JavaScript%20(4.24.0)%3B%20Browser%20(lite)"
           "&x-algolia-api-key=YmRiYzczYjlmNzMzNjc0MmRhMmIzZmRjOGJjNWY0YjJkMDY5YTkyMzAyNTk3ZDRlNzJlN2E3NTVlMWM3MTEyYnJlc3RyaWN0SW5kaWNlcz1Qb3N0X3Byb2R1Y3Rpb24lMkNQbGFjZXNfcHJvZHVjdGlvbiUyQ0NvbXBhbnlfcHJvZHVjdGlvbiZ2YWxpZFVudGlsPTE3ODQxOTc4ODg%3D"
...
def _fetch_page(page, opener=None):
    body = json.dumps({"requests": [
        {"indexName": INDEX_NAME, "query": QUERY, "params": _params(page)}]}).encode()
    req = Request(
        API_URL,
        data=body,
        method="POST",
    ...
    with THROTTLE.hold(API_URL):
        with urlopen(req, timeout=TIMEOUT) as r:
            return json.loads(r.read().decode("utf-8", errors="replace"))


def fetch_all(opener=None):
    listings = []
    page = 0
    while page < MAX_PAGES:
        try:
            data = _fetch_page(page, opener=opener)
