...
| `internships/desc_store.py` | `out/descriptions.json` — `{listing_id: html_or_text}`. Keeps the multi‑MB job bodies out of `all.json` (and off the web UI hot path). Gitignored; local-only. |
